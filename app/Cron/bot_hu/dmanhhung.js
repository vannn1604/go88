...
let spin = function(io, user){
	...
	let nohu    = false;
	// tạo kết quả
	HU.findOne({game:'dongmauanhhung', type:bet, red:true}, {}, function(err2, dataHu){
		...
		.then(result => {
				...
				if(line_win.win == 5) {
					if (line_win.type === 5) {
						...
						if (!nohu) {
							...
							bet_win += okHu;
							HU.updateOne({game:'dongmauanhhung', type:bet, red:true}, {$set:{name:'', bet:dataHu.min}}).exec();
						}else{
							okHu = Math.floor(dataHu.min-Math.ceil(dataHu.min*2/100));
							bet_win += okHu;
						}
						//io.sendInHome({pushnohu:{title:'Dòng Máu Anh Hung', name:user.name, bet:okHu}});
						nohu = true;
						DongMauAnhhung_red.create({'dichvu':'Dòng Máu Anh Hùng', 'name':user.name, 'type':2, 'win':bet_win, 'bet':bet, 'kq':1, 'line':1, 'time':new Date()});
			...
			HU.updateOne({game:'dongmauanhhung', type:bet, red:true}, {$inc:huUpdate}).exec();
