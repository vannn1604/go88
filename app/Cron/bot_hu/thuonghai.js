let ThuongHai_red = require('../../Models/LichSu_Cuoc');
...
let spin = function(io, user){
	...
	let nohu      = false;
	// tạo kết quả
	HU.findOne({game:'thuong', type:bet, red:true}, {}, function(err2, dataHu){
		...
		.then(result => {
				...
				if(!nohu && line_win.win == 10) {
					...
					if (line_win.type === 5) {
						...
						if (!nohu) {
							...
							bet_win += okHu;
							HU.updateOne({game:'thuong', type:bet, red:true}, {$set:{name:'', bet:dataHu.min}}).exec();
						}else{
						...
						nohu = true;
						ThuongHai_red.create({'name':user.name, 'type':2, 'win':bet_win, 'bet':bet, 'kq':1, 'line':1, 'time':new Date()});
			...
			HU.updateOne({game:'thuong', type:bet, red:true}, {$inc:{bet:addQuy}}).exec();
