...
let spin = function(io, user){
	...
	let nohu      = false;
	// tạo kết quả
	HU.findOne({game:'Caoboi', type:bet, red:true}, {}, function(err2, dataHu){
		...
		.then(result => {
				...
				if(!nohu && line_win.win == 10) {
					...
					if (line_win.type === 5) {
						...
						if (!nohu) {
							...
							bet_win += okHu;
							HU.updateOne({game:'Caoboi', type:bet, red:true}, {$set:{name:'', bet:dataHu.min}}).exec();
						}else{
							okHu = Math.floor(dataHu.min-Math.ceil(dataHu.min*2/100));
							bet_win += okHu;
						}
						//let Caoboi_red  = require('../../Models/Caoboi/Caoboi_red');
						/* io.sendInHome({pushnohu:{title:'Liên Minh Huyền Thoại', name:user.name, bet:okHu}}); */
						Caoboi_red.create({'dichvu':'Cao Bồi', 'name':user.name, 'type':2, 'win':bet_win, 'bet':bet, 'kq':1, 'line':1, 'time':new Date()});
						// io.sendInHome({pushnohu:{title:'Liên Minh Huyền Thoại', name:user.name, bet:okHu}}); 
						nohu = true;
			...
			HU.updateOne({game:'Caoboi', type:bet, red:true}, {$inc:{bet:addQuy}}).exec();
