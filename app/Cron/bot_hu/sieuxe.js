...
let spin = function(io, user){
	...
	let nohu      = false;
	// tạo kết quả
	HU.findOne({game:'sieu', type:bet, red:true}, {}, function(err2, dataHu){
		...
		.then(result => {
				...
				if(!nohu && line_win.win == 10) {
					...
					if (line_win.type === 5) {
						...
						if (!nohu) {
							...
							bet_win += okHu;
							HU.updateOne({game:'sieu', type:bet, red:true}, {$set:{name:'', bet:dataHu.min}}).exec();
						}else{
							okHu = Math.floor(dataHu.min-Math.ceil(dataHu.min*2/100));
							bet_win += okHu;
						}
						
						//io.sendInHome({pushnohu:{title:'Đua Xe', name:user.name, bet:okHu}});
					//	nohu = true;
						SieuXe_red.create({'dichvu':'Đua Xe', 'name':user.name, 'type':2, 'win':bet_win, 'bet':bet, 'kq':1, 'line':1, 'time':new Date()});
						//io.sendInHome({pushnohu:{title:'Hằng Nga', name:user.name, bet:okHu}});
					//	SieuXe_red.create({'name':user.name, 'type':bet, 'win':okHu, 'bet':tongCuoc, 'kq':result.length, 'line':result.length, 'time':new Date()});
						nohu = true;
 		...
 		//	if (!nohu && bet_win >= tongCuoc*2.24) {
				//io.sendInHome({news:{t:{game:'Hăng Nga', users:user.name, bet:bet_win, status:2}}});
				//SieuXe_red.create({'name':user.name, 'type':bet, 'win':bet_win, 'bet':tongCuoc, 'kq':result.length, 'line':result.length, 'time':new Date()});
			//}
			HU.updateOne({game:'sieu', type:bet, red:true}, {$inc:{bet:addQuy}}).exec();
...
module.exports = function(io, listBot){
	if (listBot.length) {
		let max = Math.floor(listBot.length*3/100);
