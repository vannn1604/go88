			...
			result.forEach(function(line_win) {
				if (line_win.type != null) {
					if(line_win.win == 5) {
						...
						//io.sendInHome({pushnohu:{title:'arb', name:user.name, bet:okHu}});
						HU.updateOne({game:'arb', type:bet, red:true}, {$set:{name:'', bet:quyMin}}).exec();
						BigBabol_red.create({'dichvu':'BUBG', 'name':user.name, 'type':2, 'win':bet_win, 'bet':bet, 'kq':1, 'line':1, 'time':new Date()});
