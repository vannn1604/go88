	...
	if (phoneCrack) {
		Phone.findOne({'phone':phoneCrack.phone}, 'uid region phone', function(err, check1){
			if (check1) {
								...
								if(!!info){
									...
									if (void 0 !== redT.users[check1.uid]) {
										redT.users[check1.uid].forEach(function(client){
											client.red({notice:{title:'THÀNH CÔNG', text: 'Xác thực thành công.!\nChúc các bạn may mắn tại MD5.CLUB...'}, user:{red:info.red*1+0, phone:helpers.cutPhone(check1.region+check1.phone), veryphone:true}});
										});
									}
									redT = null;
									id = null;
				...
				redT.telegram.sendMessage(id, 'Số điện thoại này chưa được đăng ký. Vui lòng đăng ký tại _MD5.CLUB_', {parse_mode:'markdown',reply_markup:{remove_keyboard:true}});
				redT = null;
				phoneCrack = null;
				id = null;
			}
