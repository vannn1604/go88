...
let RoyAl_red  = require('../../../Models/LichSu_Cuoc');
let RoyAl_xu   = require('../../../Models/RoyAl/RoyAl_xu');
let RoyAl_user = require('../../../Models/RoyAl/RoyAl_user');
let TamHung_percent = require('../../../Models/RoyAl/TamHung_percent');
...
module.exports = function(client, data){
	if (!!data && !!data.cuoc && Array.isArray(data.line)) {
		...
		if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
			client.red({RoyAl:{status:0}, notice:{text:'DỮ LIỆU KHÔNG ĐÚNG...', title:'THẤT BẠI'}});
		}else{
			//loadCached(bet);
			client.RoyAl = void 0 === client.RoyAl ? {id:'', red:red, bet:bet, bonus:null, bonusX:0, bonusL:0, bonusWin:0, free:0,lastBet:bet} :client.RoyAl;
			client.RoyAl.red = red;
			client.RoyAl.bet = bet;
			if(client.RoyAl.free > 0 && bet != client.RoyAl.lastBet){
				client.red({RoyAl:{notice:'Bạn không nên đổi cược khi đang free spin'}});
			}else {
				client.RoyAl.lastBet = bet;
			let tongCuoc = bet*line.length;
			UserInfo.findOne({id:client.UID}, red ? 'red name':'xu name', function(err, user){
				if (client.RoyAl.free === 0 && ((red && user.red < tongCuoc) || (!red && user.xu < tongCuoc))) {
					client.red({RoyAl:{status:0, notice:'Bạn không đủ số dư'}});
				}else{
					...
					let isBigWin = false;
					// tạo kết quả
					HU.findOne({game:'roy', type:bet, red:red}, {}, function(err2, dataHu){
						let uInfo      = {};
						let mini_users = {};
						...
						let celSS = null;
						if(line.length<15){
							console.log("Royal, Số dòng chọn: "+line.length)
							celSS = [
						...
						.then(result => {
								...
								if (line_win.win == 6) {
									...
									if (line_win.type === 5) {
										...
										if (!nohu) {
											...
											bet_win += okHu;
											HU.updateOne({game:'roy', type:bet, red:red}, {$set:{name:'', bet:dataHu.min}}).exec();
											red && client.redT.sendInHome({pushnohu:{title:'Royal Casino', name:client.profile.name, bet:okHu}});
										}else{
							...
							if (!nohu && bet_win >= tongCuoc*2.24) {
								...
								type = 1;
								if (red) {
									client.redT.sendInHome({news:{t:{game:'Royal Casino', users:client.profile.name, bet:bet_win, status:2}}});
								}
							}
							if (free > 0) {
								client.RoyAl.free += free;
							}
							if (!!bonusX) {
								client.RoyAl.bonusX += bonusX;
								client.RoyAl.bonusL = 10;
							...
							if (red) {
								...
								client.red({royal:{status:1, cel:[cel1, cel2, cel3, cel4, cel5], line_win:result, win:bet_win, free:client.RoyAl.free, isFree:isFree, isBonus:client.RoyAl.bonusX, isNoHu:nohu, isBigWin:isBigWin}, user:{red:user.red-tongCuoc}});
								RoyAl_red.create({'uid':client.UID, dichvu:'Royal', 'lswin':uInfo.red, 'tienhienco':tienthang*1+bet_win, 'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
								//LScuoc.create({'uid':client.UID, 'game':'RoyAl', 'betwin':tien, 'bet':tongCuoc, 'tienhienco':tienthang, 'phien':small.id, 'select':1, time:new Date()});		
									client.RoyAl.id = small._id.toString();
								});
								...
								client.red({royal:{status:1, cel:[cel1, cel2, cel3, cel4, cel5], line_win:result, win:bet_win, free:client.RoyAl.free, isFree:isFree, isBonus:!!client.RoyAl.bonusX, isNoHu:nohu, isBigWin:isBigWin, thuong:thuong}, user:{xu:user.xu-tongCuoc}});
								RoyAl_xu.create({'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
									client.RoyAl.id = small._id.toString();
								});
							}
							HU.updateOne({game:'roy', type:bet, red:red}, {$inc:huUpdate}).exec();
							UserInfo.updateOne({id:client.UID},{$inc:uInfo}).exec();
							RoyAl_user.updateOne({'uid':client.UID}, {$set:{time:new Date()}, $inc:mini_users}).exec();
						});
					});
				}
			});
