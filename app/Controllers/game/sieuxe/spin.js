...
let SieuXe_red  = require('../../../Models/LichSu_Cuoc');
let SieuXe_xu   = require('../../../Models/SieuXe/SieuXe_xu');
let SieuXe_user = require('../../../Models/SieuXe/SieuXe_user');
let TamHung_percent = require('../../../Models/SieuXe/SieuXe_percent');
...
module.exports = function(client, data){
	if (!!data && !!data.cuoc && Array.isArray(data.line)) {
		...
		if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
			client.red({SieuXe:{status:0}, notice:{text:'DỮ LIỆU KHÔNG ĐÚNG...', title:'THẤT BẠI'}});
		}else{
			//loadCached(bet);
			client.SieuXe = void 0 === client.SieuXe ? {id:'', red:red, bet:bet, bonus:null, bonusX:0, bonusL:0, bonusWin:0, free:0,lastBet:bet} :client.SieuXe;
			client.SieuXe.red = red;
			client.SieuXe.bet = bet;
			if(client.SieuXe.free > 0 && bet != client.SieuXe.lastBet){
				client.red({SieuXe:{notice:'Bạn không nên đổi cược khi đang free spin'}});
			}else {
				client.SieuXe.lastBet = bet;
			let tongCuoc = bet*line.length;
			UserInfo.findOne({id:client.UID}, red ? 'red name':'xu name', function(err, user){
				if (client.SieuXe.free === 0 && ((red && user.red < tongCuoc) || (!red && user.xu < tongCuoc))) {
					client.red({SieuXe:{status:0, notice:'Bạn không đủ số dư'}});
				}else{
					...
					let isBigWin = false;
					// tạo kết quả
					HU.findOne({game:'sieu', type:bet, red:red}, {}, function(err2, dataHu){
						let uInfo      = {};
						let mini_users = {};
						...
						.then(result => {
								...
								if (line_win.win == 6) {
									...
									if (line_win.type === 5) {
										...
										if (!nohu) {
											...
											bet_win += okHu;
											HU.updateOne({game:'sieu', type:bet, red:red}, {$set:{name:'', bet:dataHu.min}}).exec();
											red && client.redT.sendInHome({pushnohu:{title:'Xe Đua', name:client.profile.name, bet:okHu}});
										}else{
							...
							let tien = 0;
							if (client.SieuXe.free > 0) {
								tien = bet_win;
								client.SieuXe.free -= 1;
							}else{
							...
							if (!nohu && bet_win >= tongCuoc*2.24) {
								...
								type = 1;
								if (red) {
									client.redT.sendInHome({news:{t:{game:'Xe Đua', users:client.profile.name, bet:bet_win, status:2}}});
								}
							}
							if (free > 0) {
								client.SieuXe.free += free;
							}
							if (!!bonusX) {
								client.SieuXe.bonusX += bonusX;
								client.SieuXe.bonusL = 10;
							...
							if (red) {
								...
								client.red({sieuxe:{status:1, cel:[cel1, cel2, cel3, cel4, cel5], line_win:result, win:bet_win, free:client.SieuXe.free, isFree:isFree, isBonus:client.SieuXe.bonusX, isNoHu:nohu, isBigWin:isBigWin}, user:{red:user.red-tongCuoc}});
								SieuXe_red.create({'uid':client.UID, dichvu:'Đua Xe', 'lswin':uInfo.red, 'tienhienco':tienthang*1+bet_win, 'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
								//LScuoc.create({'uid':client.UID, 'game':'Siêu Xe', 'betwin':tien, 'bet':tongCuoc, 'tienhienco':tienthang, 'phien':small.id, 'select':1, time:new Date()});	
									client.SieuXe.id = small._id.toString();
								});
								...
								client.red({sieuxe:{status:1, cel:[cel1, cel2, cel3, cel4, cel5], line_win:result, win:bet_win, free:client.SieuXe.free, isFree:isFree, isBonus:!!client.SieuXe.bonusX, isNoHu:nohu, isBigWin:isBigWin, thuong:thuong}, user:{xu:user.xu-tongCuoc}});
								SieuXe_xu.create({'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
									client.SieuXe.id = small._id.toString();
								});
							}
							HU.updateOne({game:'sieu', type:bet, red:red}, {$inc:huUpdate}).exec();
							UserInfo.updateOne({id:client.UID},{$inc:uInfo}).exec();
							SieuXe_user.updateOne({'uid':client.UID}, {$set:{time:new Date()}, $inc:mini_users}).exec();
						});
					});
				}
			});
