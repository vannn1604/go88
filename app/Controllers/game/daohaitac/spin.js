...
let Daohaitac_xu   = require('../../../Models/Daohaitac/Daohaitac_xu');
...
function gameBonus(client, bet){
	...
	.then(result => {
		client.Daohaitac.bonus = result;
	});
}

module.exports = function(client, data){
	//console.log("Mỹ Nhân Ngư: "+data)
	if (!!data && !!data.cuoc && Array.isArray(data.line)) {
		...
		if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
			client.red({daohaitac:{status:0}, notice:{text:'DỮ LIỆU KHÔNG ĐÚNG...', title:'THẤT BẠI'}});
		}else{
			client.Daohaitac = void 0 === client.Daohaitac ? {id:'', red:red, bet:bet, bonus:null, bonusL:0, bonusWin:0, free:0} :client.Daohaitac;
			client.Daohaitac.red = red;
			client.Daohaitac.bet = bet;
			let tongCuoc = bet*line.length;
			UserInfo.findOne({id:client.UID}, red ? 'red name daily':'xu name daily', function(err, user){
				if (client.Daohaitac.free === 0 && ((red && user.red < tongCuoc) || (!red && user.xu < tongCuoc))) {
					client.red({daohaitac:{status:0, notice:'Bạn không đủ số dư'}});
				}else{
					if (user.daily != '')
						Helpers.pushDailyVIP({ daily: user.daily, reason: "Cộng tiền người chơi " + user.name + " chơi Đảo Hải Tặc", type: true, total: tongCuoc });
					let config = require('../../../../config/daohaitac.json');
					let phe = red ? 2 :4;    // Phế
					let addQuy = (tongCuoc*0.005)>>0;
					Helpers.MissionAddCurrent(client.UID, phe);
					let line_nohu = 0;
					let bet_win   = 0;
					let free      = 0;
					...
					let isBigWin  = false;
					// tạo kết quả
					HU.findOne({game:'daohaitac', type:bet, red:red}, {}, function(err2, dataHu){
						let uInfo      = {};
						let mini_users = {};
						...
						let celSS = null;
						if(line.length<15){
							console.log("Mỹ Nhân Ngư, Số dòng chọn: "+line.length)
							celSS = [
								random_T3(), 2, random_T3(),
								random_T3(), random_T3(), 0,
								random_T3(), 0, 8,
								1, 7, 3,
								random_T3(), random_T3(), 9,
							];
							console.log(celSS)
						}else{
							console.log("Mỹ Nhân Ngư, Số dòng chọn: "+line.length)
						...
						if (bonusScreen >= 5) {
							// Bonus 5
							client.Daohaitac.bonusL = 5;
							gameBonus(client, bet);
						}else if (bonusScreen == 4){
							// Bonus 4
							client.Daohaitac.bonusL = 4;
							gameBonus(client, bet);
						}else if (bonusScreen == 3){
							// Bonus 3
							client.Daohaitac.bonusL = 3;
							gameBonus(client, bet);
						}
						...
						.then(result => {
								...
								if(!nohu && line_win.win == 10) {
									...
									if (line_win.type === 5) {
										...
										if (!nohu) {
											...
											bet_win += okHu;
											HU.updateOne({game:'daohaitac', type:bet, red:red}, {$set:{name:'', bet:dataHu.min}}).exec();
											red && client.redT.sendInHome({pushnohu:{title:'Đảo Hải Tặc', name:client.profile.name, bet:okHu}});
										}else{
							...
							let tien = 0;
							if (client.Daohaitac.free > 0) {
								tien = bet_win;
								client.Daohaitac.free -= 1;
							}else{
							...
							if (!nohu && bet_win >= tongCuoc*2.24) {
								...
								type = 1;
								if (red) {
									client.redT.sendInHome({news:{t:{game:'Đảo Hải Tặc', users:client.profile.name, bet:bet_win, status:2}}});
								}
							}
							if (free > 0) {
								client.Daohaitac.free += free;
							}
							...
							if (red) {
								...
								Daohaitac_red.create({'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
								LScuoc.create({'uid':client.UID, 'game':'Đảo Hải Tặc', 'betwin':tien, 'bet':tongCuoc, 'tienhienco':tienthang, 'phien':small.id, 'select':1, time:new Date()});			
									client.red({daohaitac:{status:1, cel:[cel1, cel2, cel3, cel4, cel5], line_win:result, win:bet_win, free:client.Daohaitac.free, isFree:isFree, isBonus:client.Daohaitac.bonusL, isNoHu:nohu, isBigWin:isBigWin, phien:small.id}, user:{red:user.red-tongCuoc}});
									client.Daohaitac.id = small._id.toString();
								});
								...
								Daohaitac_xu.create({'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
									client.red({daohaitac:{status:1, cel:[cel1, cel2, cel3, cel4, cel5], line_win:result, win:bet_win, free:client.Daohaitac.free, isFree:isFree, isBonus:!!client.Daohaitac.bonusX, isNoHu:nohu, isBigWin:isBigWin, thuong:thuong, phien:small.id}, user:{xu:user.xu-tongCuoc}});
									client.Daohaitac.id = small._id.toString();
								});
							}
							HU.updateOne({game:'daohaitac', type:bet, red:red}, {$inc:huUpdate}).exec();
							UserInfo.updateOne({id:client.UID},{$inc:uInfo}).exec();
							Daohaitac_user.updateOne({'uid':client.UID}, {$set:{time:new Date()}, $inc:mini_users}).exec();
						})
					})
				}
			});
