...
function gameBonus(client, bet){
	...
	client.Candy.bonus = map.map(function(obj){
		...
		if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
			...
			UserInfo.findOne({id:client.UID}, 'red name', function(err, user){
				...
				if (client.Candy.free === 0 && user.red < tongCuoc) {
					client.red({candy:{status:0, notice:'Bạn không đủ số dư'}});
				}else{
						...
						*/
						...
						.then(result => {
								...
								if (line_win.win == 9) {
									...
									if (line_win.type === 5) {
										...
										if (!nohu) {
											...
											client.redT.sendInHome({pushnohu:{title:'Panda', name:client.profile.name, bet:okHu}});
										}else{
							...
							if (!nohu && bet_win >= tongCuoc*2.24) {
								isBigWin = true;
								//type = 1;
								bet_win >= 10000 && client.redT.sendInHome({news:{t:{game:'Panda', users:client.profile.name, bet:bet_win, status:2}}});
							}
							if (free > 0) {
							...
							LScuoc.create({'uid':client.UID, 'game':'Panda KungFu', 'betwin':tien, 'bet':tongCuoc, 'tienhienco':tienthang, 'phien':small.id, 'select':1, time:new Date()});		            
