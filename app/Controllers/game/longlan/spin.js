...
let LongLan_red  = require('../../../Models/LichSu_Cuoc');
		...
		if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
			...
			UserInfo.findOne({id:client.UID}, 'red name', function(err, user){
				if (!user || client.LongLan.free === 0 && user.red < tongCuoc) {
					client.red({longlan:{status:0, notice:'Bạn không đủ số dư'}});
				}else{
					...
					HU.findOne({game:'long', type:bet}, {}, function(err2, dataHu){
						...
						.then(result => {
							...
							if (!nohu && bet_win >= tongCuoc*2.24) {
								isBigWin = true;
								//type = 1;
								bet_win >= 10000 && client.redT.sendInHome({news:{t:{game:'Đập Hũ', users:client.profile.name, bet:bet_win, status:2}}});
							}
							if (free > 0) {
							...
							if (tien < 0){
                             ...
                             LongLan_red.create({'uid':client.UID, dichvu:'Avengers', 'lswin':uInfo.red, 'tienhienco':tienthang*1+bet_win, 'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
