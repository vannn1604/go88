...
let Zeus_red  = require('../../../Models/LichSu_Cuoc');
let Zeus_user = require('../../../Models/Zeus/Zeus_user');
let Zeus_percent = require('../../../Models/Zeus/Zeus_percent');
...
function gameBonus(client, bet){
	...
	.then(result => {
		client.Zeus.bonus = result;
	});
}

function loadCached(bet){
		randomPer = Zeus_percent.findOne({type:bet}).exec();
		if(!!randomPer){
		...
		if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
			...
			UserInfo.findOne({id:client.UID}, 'red name', function(err, user){
				if (!user || client.Zeus.free === 0 && user.red < tongCuoc) {
					client.red({Zeus:{status:0, notice:'Bạn không đủ số dư'}});
				}else{
					...
					// tạo kết quả
					HU.findOne({game:'Zeus', type:bet}, {}, function(err2, dataHu){
						if (dataHu === null){
							return void 0;
						}
						let uInfo      = {hu:0};
						...
						.then(result => {
								...
								if(!nohu && line_win.win == 10) {
									...
									if (line_win.type === 5) {
										...
										if (!nohu) {
											...
											bet_win += okHu;
											HU.updateOne({game:'Zeus', type:bet}, {$set:{name:'', bet:dataHu.min}}).exec();
											red && client.redT.sendInHome({pushnohu:{title:'Liên Minh Huyền Thoại', name:client.profile.name, bet:okHu}});
										}else{
							...
							if (!nohu && bet_win >= tongCuoc*2.24) {
								...
								//type = 1;
								//if (red) {
									bet_win >= 10000 && client.redT.sendInHome({news:{t:{game:'Liên Minh Huyền Thoại', users:client.profile.name, bet:bet_win, status:2}}});
								//}
							}
							if (free > 0) {
							...
							let thuong = 0;
								...
								Zeus_red.create({'uid':client.UID, dichvu:'Liên Minh Huyền Thoại', 'lswin':uInfo.red, 'tienhienco':tienthang*1+bet_win, 'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
									//LScuoc.create({'uid':client.UID, 'game':'Liên Minh Huyền Thoại', 'betwin':tien, 'bet':tongCuoc, 'tienhienco':tienthang, 'phien':small.id, 'select':1, time:new Date()});
							...
							HU.updateOne({game:'Zeus', type:bet}, {$inc:huUpdate}).exec();
