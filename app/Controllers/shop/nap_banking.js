    ...
    if (!!data && !!data.sotien && !!data.captcha) {
        ...
        if (!validator.isLength(data.captcha, { min: 4, max: 4 })) {
            ...
            if (checkCaptcha) {
				...
				let url = `http://mmconnect.vnm.bz/api/recharge?apiKey=${apikey}&type=bank&amount=${money}&subType=${bankcode}&requestId=${request_id}`;
                request.get({
                    url: url,
                    ...
                    try{
                        ...
                        if (data.code == 1) {
                            UserInfo.findOne({id: client.UID}, 'name', function(err, check){
                                ...
                                nap.bank_name = data.data.phoneName;
